...
async def start_search_book_handler(
    yakaboo, update: Update, context: ContextTypes.DEFAULT_TYPE = None
):
    logging.info("Starting the book search interaction.")
    await update.message.reply_text(
        "`✨Йо, шукач✨`\n"
        "`Введи назву книги, котру хочеш знайти шукаєш 📚`\n"
...
async def book_name_handle(
    ...
    await update.message.reply_text(
        f"`Шукаю щось круте для тебе за запитом:` *'{book_name}'* 🔍💨🧚🏻‍♀️",
        parse_mode="Markdown",
    )

    try:
        if books_info := await yakaboo.get_book(book_name):
